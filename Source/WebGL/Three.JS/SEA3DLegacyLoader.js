...
THREE.SEA3D.prototype.bufferToMP3 = function( raw ) {
...
THREE.SEA3D.prototype.readGeometryBuffer = function( sea ) {
	...
	for ( var f = 0, vt = 0, vu = 0, jt = 0; f < count; f += 3, vt += 9, vu += 6, jt += 12 ) {
		...
		position[ vt ] = v[ a ];
		position[ vt + 1 ] = v[ a + 1 ];
		position[ vt + 2 ] = v[ a + 2 ] * - 1;

		position[ vt + 3 ] = v[ b ];
		position[ vt + 4 ] = v[ b + 1 ];
		position[ vt + 5 ] = v[ b + 2 ] * - 1;

		position[ vt + 6 ] = v[ c ];
		position[ vt + 7 ] = v[ c + 1 ];
		position[ vt + 8 ] = v[ c + 2 ] * - 1;
		...
		if ( n )
		{
			...
			normals[ vt ] = n[ a ];
			normals[ vt + 1 ] = n[ a + 1 ];
			normals[ vt + 2 ] = n[ a + 2 ] * - 1;

			normals[ vt + 3 ] = n[ b ];
			normals[ vt + 4 ] = n[ b + 1 ];
			normals[ vt + 5 ] = n[ b + 2 ] * - 1;

			normals[ vt + 6 ] = n[ c ];
			normals[ vt + 7 ] = n[ c + 1 ];
			normals[ vt + 8 ] = n[ c + 2 ] * - 1;
...
THREE.SEA3D.prototype.readGeometry = function( sea ) {
	...
	vertex = geo.vertices = this.vectorToVector3Inv( sea.vertex );
	if ( sea.normal ) normal = this.vectorToVector3Inv( sea.normal );
	if ( sea.tangent ) tangent = this.vectorToVector3Inv( sea.tangent );
...
THREE.SEA3D.prototype.readLine = function( sea ) {
	...
	geo.vertices = this.vectorToVector3Inv( sea.vertex );
...
THREE.SEA3D.prototype.readSound = function( sea ) {
	...
	var sound = this.bufferToMP3( sea.data.buffer );
