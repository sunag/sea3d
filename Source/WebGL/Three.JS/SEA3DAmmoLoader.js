...
THREE.SEA3D.prototype.readGeometryShape = function( sea ) {

	var triMesh = THREE.AMMO.createTriMesh( sea.geometry.tag );
...
THREE.SEA3D.prototype.readGeometryShape = function( sea ) {
	...
	var triMesh = THREE.AMMO.createTriMesh( sea.geometry.tag, sea.subGeometryIndex );
...
THREE.SEA3D.prototype.readStaticGeometryShape = function( sea ) {

	var triMesh = AMMO.createTriMesh( sea.geometry.tag, sea.subGeometryIndex );
...
THREE.SEA3D.prototype.readRigidBody = function( sea ) {
	...
	THREE.AMMO.addRigidBody( rb, sea.target ? sea.target.tag : undefined, sea.offset );
...
THREE.SEA3D.prototype.readCarController = function( sea ) {
	...
	sea.suspensionStiffness = 40;
	sea.suspensionDamping = 2.5;
	sea.suspensionCompression = 2.5;
	sea.maxSuspensionTravelCm = 500;
	sea.maxSuspensionForce = 6000;
	sea.frictionSlip = 0.96;
	...
	THREE.AMMO.addRigidBody( body, sea.target ? sea.target.tag : undefined, sea.offset );
...
THREE.SEA3D.EXTENSIONS.push( function() {
	...
	if ( this.config.physics ) {
		...
		this.file.typeRead[ SEA3D.GeometryShape.prototype.type ] = this.readGeometryShape;
		this.file.typeRead[ SEA3D.StaticGeometryShape.prototype.type ] = this.readStaticGeometryShape;
