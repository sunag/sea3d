package sunag.sea3d.modules
{
	...
	{
		protected var _context:AssetLoaderContext;
		...
		public function getContext():AssetLoaderContext
		{
			if (!_context) _context = new AssetLoaderContext();
			return _context;
		}
		
		public function getSparticle(name:String):SparticleLoader
		{
			return sea.object[name + ".sptc"];
		}
		...
		protected function readSparticle(sea:SEASparticle):void
		{
			...
			var context:AssetLoaderContext = getContext();
			
			for each(var file:SEAObject in sea.deps)
			{
				context.mapUrlToData(file.fullname, file.data);
			}
			
			sparticle.loadData(sea.source, '', context);
			
			_sparticles ||=  new Vector.<SparticleLoader>();
			_sparticles.push(this.sea.object[sea.name + '.sptc'] = sea.tag = sparticle);
		}
