...
THREE.ReflectNode.prototype.generate = function( builder, output ) {
	...
	switch ( this.scope ) {
		...
		case THREE.ReflectNode.VECTOR:
			...
			builder.material.addFragmentNode( [ 
			
				'vec3 reflectVec = inverseTransformDirection( reflect( -geometry.viewDir, geometry.normal ), viewMatrix );'
		...
		case THREE.ReflectNode.CUBE:
			...
			result = 'queryReflectVec';
			
			break;

		case THREE.ReflectNode.SPHERE:
		
			result = '(normalize((viewMatrix * vec4(' + new THREE.ReflectNode( THREE.ReflectNode.VECTOR ).build( builder, 'v3' ) + ', 0.0 )).xyz + vec3(0.0,0.0,1.0)).xy * 0.5 + 0.5)';
