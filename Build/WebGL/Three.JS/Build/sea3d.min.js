...
var SEA3D={VERSION:17E3,getVersion:function(){var a=SEA3D.VERSION.toString(),b=a.length;return a.substring(0,b-4)+"."+a.substring(b-4,b-3)+"."+a.substring(b-3,b-2)+"."+parseFloat(a.substring(b-2,b)).toString()}};console.log("SEA3D "+SEA3D.getVersion());SEA3D.Timer=function(){this.time=this.start=this.getTime()};SEA3D.Timer.prototype.getDeltaTime=function(){return this.getTime()-this.time};SEA3D.Timer.prototype.getTime=function(){return Date.now()};
SEA3D.Timer.prototype.getElapsedTime=function(){return this.getTime()-this.start};SEA3D.Timer.prototype.update=function(){this.time=this.getTime()};SEA3D.Stream=function(a){this.position=0;this.set(a||new Uint8Array)};SEA3D.Stream.bufferToBase64=function(a){for(var b=0,c=a.length,d="";b<c;)d+=String.fromCharCode(a[b++]);return window.btoa(d)};SEA3D.Stream.prototype.TWOeN23=Math.pow(2,-23);SEA3D.Stream.prototype.TWOeN52=Math.pow(2,-52);SEA3D.Stream.prototype.pow=Math.pow;
SEA3D.Stream.prototype.set=function(a){this.buffer=a;this.length=a.length};SEA3D.Stream.prototype.readByte=function(){return this.buffer[this.position++]<<24>>24};SEA3D.Stream.prototype.readUByte=function(){return this.buffer[this.position++]};SEA3D.Stream.prototype.readUBytes=function(a){for(var b=new Uint8Array(a),c=0;c<a;++c)b[c]=this.buffer[this.position++];return b};SEA3D.Stream.prototype.readBool=function(){return 0!=this.readUByte()};
SEA3D.Stream.prototype.readShort=function(){var a=this.readUShort();return a>Math.pow(2,15)-1?a-Math.pow(2,16):a};SEA3D.Stream.prototype.readUShort=function(){return this.readUByte()|this.readUByte()<<8};SEA3D.Stream.prototype.readUInt24=function(){return this.readUByte()|this.readUByte()<<8|this.readUByte()<<16};SEA3D.Stream.prototype.readInt=function(){var a=this.readUInt();return a>Math.pow(2,31)-1?a-Math.pow(2,32):a};
SEA3D.Stream.prototype.readUInt=function(){return this.readUByte()|this.readUByte()<<8|this.readUByte()<<16|this.readUByte()<<24};SEA3D.Stream.prototype.readUInteger=function(){var a=this.readUByte(),b=a&127;0!=(a&128)&&(a=this.readUByte(),b|=(a&127)<<7,0!=(a&128)&&(a=this.readUByte(),b|=(a&127)<<13));return b};
...
SEA3D.GeometryDelta=function(a,b,c){var d,e,f,h;this.name=a;this.data=b;this.sea=c;this.attrib=b.readUShort();this.numVertex=b.readUInteger();this.length=3*this.numVertex;this.attrib&1?(e=function(){return b.readByte()},f=127.5):(e=function(){return b.readShort()},f=32767.5);if(this.attrib&4)for(d=b.readFloat(),this.normal=[],a=0;a<this.length;)this.normal[a++]=e()/f*d;if(this.attrib&8)for(d=b.readFloat(),this.tangent=[],a=0;a<this.length;)this.tangent[a++]=e()/f*d;if(this.attrib&32){this.uv=[];this.uv.length=
b.readUByte();var g=2*this.numVertex;for(a=0;a<this.uv.length;)for(d=b.readFloat(),this.uv[a++]=h=[],c=0;c<g;)h[c++]=e()/f*d}if(this.attrib&64){this.jointPerVertex=b.readUByte();c=this.numVertex*this.jointPerVertex;this.joint=[];this.weight=[];for(a=0;a<c;)this.joint[a++]=b.readUInteger();for(a=0;a<c;)this.weight[a++]=e()/f*1}if(this.attrib&128)for(a=b.readUByte(),d=((a&64)>>6|(a&128)>>6)+1,h=4*this.numVertex,this.color=[],this.color.length=a&15,a=0;a<this.color.length;a++){g=[];switch(d){case 1:for(c=
0;c<h;)g[c++]=b.readUByte()/255,g[c++]=0,g[c++]=0,g[c++]=1;break;case 2:for(c=0;c<h;)g[c++]=b.readUByte()/255,g[c++]=b.readUByte()/255,g[c++]=0,g[c++]=1;break;case 3:for(c=0;c<h;)g[c++]=b.readUByte()/255,g[c++]=b.readUByte()/255,g[c++]=b.readUByte()/255,g[c++]=1;break;case 4:for(c=0;c<h;)g[c++]=b.readUByte()/255,g[c++]=b.readUByte()/255,g[c++]=b.readUByte()/255,g[c++]=b.readUByte()/255}this.color[a]=g}d=b.readFloat();this.vertex=[];for(a=0;a<this.length;)this.vertex[a++]=e()/f*d;this.indexes=[];this.indexes.length=
b.readUByte();if(this.attrib&2)for(a=0;a<this.indexes.length;a++)for(f=b.readUInteger(),this.indexes[a]=e=[],c=0;c<f;c++){d=b.readUInteger();h=b.readUInteger();var g=b.readUInteger(),k=b.readUInteger();e.push(d);e.push(h);e.push(g);0<k&&(e.push(g),e.push(k+1),e.push(d))}else for(f=3*b.readUInteger(),a=0;a<this.indexes.length;a++)for(this.indexes[a]=e=[],c=0;c<f;)e[c++]=b.readUInteger()};SEA3D.GeometryDelta.prototype.type="geDL";
...
SEA3D.Actions=function(a,b,c){this.name=a;this.data=b;this.sea=c;this.count=b.readUInt();this.action=[];for(a=0;a<this.count;a++){var d=b.readUByte(),e=b.readUShort(),f=b.readUShort(),h=b.position,g=this.action[a]={kind:e};d&1&&(g.range=[b.readUInt(),b.readUInt()]);d&2&&(g.time=b.readUInt());d&4&&(g.intrpl=SEA3D.DataTable.INTERPOLATION_TABLE[b.readUByte()],0==g.intrpl.indexOf("back.")?g.intrplParam0=b.readFloat():0==g.intrpl.indexOf("elastic.")&&(g.intrplParam0=b.readFloat(),g.intrplParam1=b.readFloat()));
switch(e){case SEA3D.Actions.RTT_TARGET:g.source=c.getObject(b.readUInt());g.target=c.getObject(b.readUInt());break;case SEA3D.Actions.LOOK_AT:g.source=c.getObject(b.readUInt());g.target=c.getObject(b.readUInt());break;case SEA3D.Actions.PLAY_SOUND:g.sound=c.getObject(b.readUInt());g.offset=b.readUInt();break;case SEA3D.Actions.PLAY_ANIMATION:g.object=c.getObject(b.readUInt());g.name=b.readUTF8();break;case SEA3D.Actions.FOG:g.color=b.readUInt24();g.min=b.readFloat();g.max=b.readFloat();break;case SEA3D.Actions.ENVIRONMENT:g.texture=
c.getObject(b.readUInt());break;case SEA3D.Actions.ENVIRONMENT_COLOR:g.color=b.readUInt24();break;case SEA3D.Actions.CAMERA:g.camera=c.getObject(b.readUInt());break;default:console.log('Action "'+e+'" not found.')}b.position=h+f}};SEA3D.Actions.SCENE=0;SEA3D.Actions.ENVIRONMENT_COLOR=1;SEA3D.Actions.ENVIRONMENT=2;SEA3D.Actions.FOG=3;SEA3D.Actions.PLAY_ANIMATION=4;SEA3D.Actions.PLAY_SOUND=5;SEA3D.Actions.ANIMATION_AUDIO_SYNC=6;SEA3D.Actions.LOOK_AT=7;SEA3D.Actions.RTT_TARGET=8;
SEA3D.Actions.CAMERA=9;SEA3D.Actions.prototype.type="act";
SEA3D.Script={DETAILED:!1,readScriptList:function(a,b){for(var c=[],d=a.readUByte(),e=0;e<d;){var f=a.readUByte(),h,g={};g.priority=f&1|f&2;if(f&4){var k,l;h=a.readUByte();if(SEA3D.Script.DETAILED)for(g.params=[],k=0;k<h;k++){l=a.readUTF8();var n=a.readUByte();g.params[k]={name:l,type:n,data:SEA3D.DataTable.readToken(n,a,b)}}else for(g.params={},k=0;k<h;k++)l=a.readUTF8(),g.params[l]=SEA3D.DataTable.readObject(a,b)}f&8&&(g.method=a.readUTF8());g.tag=b.getObject(a.readUInt());c[e++]=g}return c}};
SEA3D.Properties=function(a,b,c){this.name=a;this.data=b;this.sea=c;this.tag=SEA3D.Properties.readProperties(b,c.objects,a)};SEA3D.Properties.readProperties=function(a,b,c){var d,e,f;if(SEA3D.Properties.DETAILED)for(d=a.readUByte(),props=[],c=0;c<d;c++)f=a.readUTF8(),e=a.readUByte(),props[c]={name:f,type:e,value:SEA3D.DataTable.readToken(e,a,b)};else for(d=a.readUByte(),props={},c&&(props.__name__=c),c=0;c<d;c++)f=a.readUTF8(),props[f]=SEA3D.DataTable.readObject(a,b);return props};
SEA3D.Properties.DETAILED=!1;SEA3D.Properties.prototype.type="prop";SEA3D.FileInfo=function(a,b,c){this.name=a;this.data=b;this.sea=c;c.info=this.tag=SEA3D.Properties.readProperties(b,c.objects,a)};SEA3D.FileInfo.prototype.type="info";SEA3D.JavaScript=function(a,b,c){this.name=a;this.data=b;this.sea=c;this.src=b.readUTF(b.length)};SEA3D.JavaScript.prototype.type="js";SEA3D.GLSL=function(a,b,c){this.name=a;this.data=b;this.sea=c;this.src=b.readUTF(b.length)};SEA3D.GLSL.prototype.type="glsl";
...
SEA3D.Animation.readAnimationList=function(a,b){for(var c=[],d=a.readUByte(),e=0;e<d;){var f=a.readUByte(),h={};h.relative=0!=(f&1);f&2&&(h.timeScale=a.readFloat());h.tag=b.getObject(a.readUInt());c[e++]=h}return c};SEA3D.Animation.prototype.type="anm";
...
SEA3D.Morph=function(a,b,c){this.name=a;this.data=b;this.sea=c;SEA3D.GeometryBase(this);a=0!=(this.attrib&2);c=0!=(this.attrib&4);var d=3*this.numVertex,e=b.readUShort();this.node=[];var f,h;for(f=0;f<e;f++){var g=b.readUTF8();if(a){var k=[];for(h=0;h<d;)k[h++]=b.readFloat()}if(c){var l=[];for(h=0;h<d;)l[h++]=b.readFloat()}this.node[f]={vertex:k,normal:l,name:g}}};SEA3D.Morph.prototype.type="mph";
...
power:b.readFloat(),blendMode:b.readBlendMode()};break;case SEA3D.Material.LIGHT_MAP:g={texture:c.getObject(b.readUInt()),channel:b.readUByte(),blendMode:b.readBlendMode()};break;case SEA3D.Material.DETAIL_MAP:g={texture:c.getObject(b.readUInt()),scale:b.readFloat(),blendMode:b.readBlendMode()};break;case SEA3D.Material.CEL:g={color:b.readUInt24(),levels:b.readUByte(),size:b.readFloat(),specularCutOff:b.readFloat(),smoothness:b.readFloat()};break;case SEA3D.Material.TRANSLUCENT:g={color:b.readUInt24(),
translucency:b.readFloat(),scattering:b.readFloat()};break;case SEA3D.Material.BLEND_NORMAL_MAP:k=b.readUByte();g={texture:c.getObject(b.readUInt()),secondaryTexture:c.getObject(b.readUInt())};k&1?(g.offsetX0=b.readFloat(),g.offsetY0=b.readFloat(),g.offsetX1=b.readFloat(),g.offsetY1=b.readFloat()):g.offsetX0=g.offsetY0=g.offsetX1=g.offsetY1=0;g.animate=k&2;break;case SEA3D.Material.MIRROR_REFLECTION:g={texture:c.getObject(b.readUInt()),alpha:b.readFloat()};break;case SEA3D.Material.AMBIENT_MAP:g=
{texture:c.getObject(b.readUInt())};break;case SEA3D.Material.ALPHA_MAP:g={texture:c.getObject(b.readUInt())};break;case SEA3D.Material.EMISSIVE_MAP:g={texture:c.getObject(b.readUInt())};break;case SEA3D.Material.VERTEX_COLOR:g={blendMode:b.readBlendMode()};break;case SEA3D.Material.WRAP_LIGHTING:g={color:b.readUInt24(),strength:b.readFloat()};break;case SEA3D.Material.COLOR_REPLACE:k=b.readUByte();g={red:b.readUInt24(),green:b.readUInt24(),blue:b.readUInt24()};k&1&&(g.mask=c.getObject(b.readUInt()));
k&2&&(g.alpha=b.readFloat());break;case SEA3D.Material.REFLECTION_SPHERICAL:g={texture:c.getObject(b.readUInt()),alpha:b.readFloat()};break;default:console.warn("SEA3D: MaterialTechnique not found:",e.toString(16));b.position=h+f;continue}g.kind=e;this.technique.push(g);b.position=h+f}};SEA3D.Material.DEFAULT=0;SEA3D.Material.COMPOSITE_TEXTURE=1;SEA3D.Material.DIFFUSE_MAP=2;SEA3D.Material.SPECULAR_MAP=3;SEA3D.Material.REFLECTION=4;SEA3D.Material.REFRACTION=5;SEA3D.Material.NORMAL_MAP=6;
...
THREE.SEA3D.prototype.vectorToVector3=function(a){for(var b=[],c=0,d=0;c<a.length;)b[d++]=new THREE.Vector3(a[c++],a[c++],a[c++]);return b};THREE.SEA3D.prototype.vectorToVector3Inv=function(a){for(var b=[],c=0,d=0;c<a.length;)b[d++]=new THREE.Vector3(a[c++],a[c++],-a[c++]);return b};
THREE.SEA3D.prototype.flipMatrixScale=function(a){a.decompose(THREE.SEA3D.VECBUF0,THREE.SEA3D.QUABUF0,THREE.SEA3D.VECBUF1);THREE.SEA3D.VECBUF1.z=-THREE.SEA3D.VECBUF1.z;a.compose(THREE.SEA3D.VECBUF0,THREE.SEA3D.QUABUF0,THREE.SEA3D.VECBUF1);return a};
...
THREE.SEA3D.prototype.updateAnimationSet=function(a){var b=a.animation.animationSet;if(!b.flip){for(var c=b.animations,d=0;d<c.length;d++){var e=c[d].dataList,f=[],h;for(h=0;h<e.length;h++){var g=e[d],k=e[d].data,l=g.kind,g=k.length/g.blockLength;switch(l){case SEA3D.Animation.POSITION:case SEA3D.Animation.ROTATION:f.push({kind:l,numFrames:g,raw:k})}}if(0<f.length){g=f[0].numFrames;e=THREE.SEA3D.CONTAINER;e.position.set(0,0,0);e.rotation.set(0,0,0);e.scale.set(1,1,1);a.add(e);h=0;for(var n;h<g;h++){for(n=
0;n<f.length;n++)switch(k=f[n].raw,l=f[n].kind,l){case SEA3D.Animation.POSITION:l=3*h;e.position.set(k[l],k[l+1],k[l+2]);break;case SEA3D.Animation.ROTATION:l=4*h,e.quaternion.set(k[l],k[l+1],k[l+2],k[l+3])}this.updateMatrix(e);for(n=0;n<f.length;n++)switch(k=f[n].raw,l=f[n].kind,l){case SEA3D.Animation.POSITION:l=3*h;k[l]=e.position.x;k[l+1]=e.position.y;k[l+2]=e.position.z;break;case SEA3D.Animation.ROTATION:l=4*h,k[l]=e.quaternion.x,k[l+1]=e.quaternion.y,k[l+2]=e.quaternion.z,k[l+3]=e.quaternion.w}}a.remove(e)}}b.flip=
!0}};THREE.SEA3D.prototype.vectorToColor=function(a){for(var b=[],c=0,d=0,e,f,h,g;c<a.length;)e=255*a[c++],f=255*a[c++],h=255*a[c++],g=255*a[c++],b[d++]=new THREE.Color(g<<24|e<<16|f<<8|h);return b};THREE.SEA3D.prototype.vectorToUV=function(a){for(var b=[],c=0;c<a.length;c++)for(var d=b[c]=[],e=a[c],f=0,h=0;f<e.length;f+=2)d[h++]=new THREE.Vector2(e[f],e[f+1]);return b};THREE.SEA3D.prototype.toVector3=function(a){return new THREE.Vector3(a.x,a.y,a.z)};
...
THREE.SEA3D.prototype.addSceneObject=function(a){var b=a.tag;b.props=a.properties;if(a.scripts)for(var c=0;c<a.scripts.length;c++){var d=a.scripts[c];"js"==d.tag.type&&this.runJS(b,d)}a.parent?a.parent.tag.add(b):this.config.container?this.config.container.add(b):this.container.add(b)};THREE.SEA3D.prototype.bufferToTexture=function(a){return"data:image/png;base64,"+SEA3D.Stream.bufferToBase64(a)};THREE.SEA3D.prototype.bufferToSound=function(a){return"data:audio/mp3;base64,"+SEA3D.Stream.bufferToBase64(a)};
THREE.SEA3D.prototype.applyDefaultAnimation=function(a,b){for(var c=a.tag,d=0,e=a.animations?a.animations.length:0;d<e;d++){var f=a.animations[d];switch(f.tag.type){case SEA3D.Animation.prototype.type:return c.animation=new b(c,f.tag.tag),c.animation.setRelative(f.relative),this.config.flip&&this.updateAnimationSet(c),this.config.autoPlay&&c.animation.play(c.animation.getStateNameByIndex(0)),c.animation}}};
THREE.SEA3D.prototype.readAnimation=function(a){for(var b=new SEA3D.AnimationSet,c=0;c<a.sequence.length;c++){for(var d=a.sequence[c],e=new SEA3D.AnimationNode(d.name,a.frameRate,d.count,d.repeat,d.intrpl),f=0;f<a.dataList.length;f++){var h=a.dataList[f];e.addData(new SEA3D.AnimationData(h.kind,h.type,h.data,d.start*h.blockSize))}b.addAnimation(e)}this.animationSets=this.animationSets||[];this.animationSets.push(this.objects[a.name+".#anm"]=a.tag=b)};
...
THREE.SEA3D.prototype.readGeometryBuffer=function(a){var b=a.indexes[0],c=b.length,d=new THREE.BufferGeometry,e,f,h,g,k;d.attributes={index:{itemSize:1,array:e=new Uint16Array(c)},position:{itemSize:3,array:f=new Float32Array(3*c)},uv:{itemSize:2,array:g=new Float32Array(2*c)}};var l,n,q,u=a.vertex,t=a.normal,x=a.tangent,y=a.uv?a.uv[0]:void 0,v=a.color?a.color[0]:void 0;t&&(d.attributes.normal={itemSize:3,array:h=new Float32Array(3*c)});x&&(d.attributes.tangent={itemSize:3,array:h=new Float32Array(3*
c)});v&&(d.attributes.color={itemSize:3,array:k=new Float32Array(3*c)});for(var r=this.config.flip?-1:1,p=0,m=0,w=0;p<c;p+=3,m+=9,w+=6)l=3*b[p],n=3*b[p+2],q=3*b[p+1],f[m]=u[l],f[m+1]=u[l+1],f[m+2]=u[l+2]*r,f[m+3]=u[n],f[m+4]=u[n+1],f[m+5]=u[n+2]*r,f[m+6]=u[q],f[m+7]=u[q+1],f[m+8]=u[q+2]*r,t&&(h[m]=t[l],h[m+1]=t[l+1],h[m+2]=t[l+2]*r,h[m+3]=t[n],h[m+4]=t[n+1],h[m+5]=t[n+2]*r,h[m+6]=t[q],h[m+7]=t[q+1],h[m+8]=t[q+2]*r),x&&(tangent[m]=x[l],tangent[m+1]=x[l+1],tangent[m+2]=x[l+2]*r,tangent[m+3]=x[n],tangent[m+
4]=x[n+1],tangent[m+5]=x[n+2]*r,tangent[m+6]=x[q],tangent[m+7]=x[q+1],tangent[m+8]=x[q+2]*r),y?(l=2*b[p],n=2*b[p+2],q=2*b[p+1],g[w]=y[l],g[w+1]=y[l+1],g[w+2]=y[n],g[w+3]=y[n+1],g[w+4]=y[q],g[w+5]=y[q+1]):(g[w]=0,g[w+1]=0,g[w+2]=0,g[w+3]=1,g[w+4]=1,g[w+5]=1),v&&(l=4*b[p],n=4*b[p+2],q=4*b[p+1],k[m]=v[l],k[m+1]=v[l+1],k[m+2]=v[l+2],k[m+3]=v[n],k[m+4]=v[n+1],k[m+5]=v[n+2],k[m+6]=v[q],k[m+7]=v[q+1],k[m+8]=v[q+2]),e[p]=p,e[p+1]=p+1,e[p+2]=p+2;65534<=a.numVertex&&console.warn("SEA3D: Big Mesh is not compatible with geometry buffer.");
t||d.computeVertexNormals();this.config.tangent&&!a.tangent&&d.computeTangents();this.config.bounding&&(d.computeBoundingBox(),d.computeBoundingSphere());d.name=a.name;a.tag=d};
THREE.SEA3D.prototype.readGeometry=function(a){var b,c,d,e=new THREE.Geometry,f,h,g,k;e.vertices=this.config.flip?this.vectorToVector3Inv(a.vertex):this.vectorToVector3(a.vertex);a.normal&&(f=this.config.flip?this.vectorToVector3Inv(a.normal):this.vectorToVector3(a.normal));a.tangent&&(h=this.config.flip?this.vectorToVector3Inv(a.tangent):this.vectorToVector3(a.tangent));a.color&&(g=this.vectorToColor(a.color[0]));if(a.uv)for(k=this.vectorToUV(a.uv),d=0;d<k.length;d++)e.faceVertexUvs[d]=[];for(b=
0;b<a.indexes.length;b++){var l=a.indexes[b],n=l.length/3;for(c=0;c<n;c++){d=3*c;var q,u,t;q=l[d];t=l[d+1];u=l[d+2];d=new THREE.Face3(q,u,t,f?[f[q],f[u],f[t]]:void 0,g?[g[q],g[u],g[t]]:void 0,b);h&&(d.vertexTangents=[h[q],h[u],h[t]]);e.faces.push(d);if(k)for(d=0;d<k.length;d++)e.faceVertexUvs[d].push([k[d][q],k[d][u],k[d][t]]);else e.faceVertexUvs[0].push([new THREE.Vector2(0,0),new THREE.Vector2(0,1),new THREE.Vector2(1,1)])}}if(a.joint)if(f=[0,0,0,0],h=[0,0,0,0],g=a.jointPerVertex,4<g)for(console.warn("SEA3D: Joint Per Vertex can not be greater than 4 (currently "+
a.jointPerVertex+"). Using compression for joints.\nTip: Use SEA3D Studio for automatic compression."),d=0;d<a.joint.length;d+=g){k=[0];for(c=1;c<g;c++)l=a.weight[d+c],l>a.weight[d+k[0]]?k.unshift(c):k.push(c);l=(1-(a.weight[d+k[0]]+a.weight[d+k[1]]+a.weight[d+k[2]]+a.weight[d+k[3]]))/4;for(c=0;4>c;c++)b=k[c],f[c]=a.joint[d+b],h[c]=a.weight[d+b]+l;e.skinIndices.push(new THREE.Vector4(f[0],f[1],f[2],f[3]));e.skinWeights.push(new THREE.Vector4(h[0],h[1],h[2],h[3]))}else for(d=0;d<a.joint.length;d+=
g){for(c=0;c<g;c++)f[c]=a.joint[d+c],h[c]=a.weight[d+c];e.skinIndices.push(new THREE.Vector4(f[0],f[1],f[2],f[3]));e.skinWeights.push(new THREE.Vector4(h[0],h[1],h[2],h[3]))}a.normal||(e.computeFaceNormals(),e.computeVertexNormals());this.config.tangent&&!a.tangent&&e.computeTangents();this.config.bounding&&(e.computeBoundingBox(),e.computeBoundingSphere());e.name=a.name;a.tag=e};
...
THREE.SEA3D.prototype.readImage=function(a){var b=new Image,c=new THREE.Texture,d=this;c.name=a.name;c.wrapS=c.wrapT=THREE.RepeatWrapping;c.flipY=!1;b.onload=function(){if(!d.isPowerOfTwo(b.width)||!d.isPowerOfTwo(b.height)){var a=d.nearestPowerOfTwo(b.width),f=d.nearestPowerOfTwo(b.height),h=document.createElement("canvas");h.width=a;h.height=f;h.getContext("2d").drawImage(b,0,0,a,f);b=h}c.image=b;c.needsUpdate=!0};b.src=this.bufferToTexture(a.data.buffer);this.textures=this.textures||[];this.textures.push(this.objects["tex/"+
a.name]=a.tag=c)};THREE.SEA3D.prototype.readCubeMap=function(a){var b=[],c=new THREE.Texture,d=[];d[0]=a.faces[1];d[1]=a.faces[0];d[2]=a.faces[3];d[3]=a.faces[2];d[4]=a.faces[5];d[5]=a.faces[4];b.loadedCount=0;c.name=a.name;c.image=b;c.flipY=!1;for(var e=0,f=d.length;e<f;++e){var h=new Image;b[e]=h;h.onload=function(){6==++b.loadedCount&&(c.needsUpdate=!0)};h.src=this.bufferToTexture(d[e].buffer)}this.cubmaps=this.cubmaps||[];this.cubmaps.push(this.objects["cmap/"+a.name]=a.tag=c)};
...
THREE.SEA3D.prototype.readJavaScript=function(a){try{var b="("+a.src.replace(/"%CUSTOM%"/g,'var watch = $GLOBAL["watch"],\nsea3d = $GLOBAL["sea3d"],\nscene = $GLOBAL["scene"],\nprint = $GLOBAL["print"];')+")";this.script=eval(b)()}catch(c){console.error('SEA3D JavaScript: Error running "'+a.name+'".'),console.error(c)}};THREE.SEA3D.prototype.readGLSL=function(a){this.glsl=this.glsl||[];this.glsl.push(this.objects["glsl/"+a.name]=a.tag=a.src)};
...
THREE.SEA3D.prototype.readSkeleton=function(a){for(var b=[],c=new THREE.Matrix4,d=new THREE.Matrix4,e=new THREE.Matrix4,f=new THREE.Vector3,h=new THREE.Quaternion,g=0;g<a.joint.length;g++){var k=a.joint[g];c.elements=k.inverseBindMatrix;d.getInverse(c);this.flipMatrix(d);-1<k.parentIndex&&(c.elements=a.joint[k.parentIndex].inverseBindMatrix,e.getInverse(c),this.flipMatrix(e),e.getInverse(e),d.multiplyMatrices(e,d));f.setFromMatrixPosition(d);h.setFromRotationMatrix(d);b[g]={name:k.name,pos:[f.x,f.y,
f.z],rotq:[h.x,h.y,h.z,h.w],parent:k.parentIndex}}a.tag=b};THREE.SEA3D.prototype.readSkeletonLocal=function(a){for(var b=[],c=0;c<a.joint.length;c++){var d=a.joint[c];b[c]={name:d.name,pos:[d.x,d.y,d.z],rotq:[d.qx,d.qy,d.qz,d.qw],parent:d.parentIndex}}a.tag=b};THREE.SEA3D.prototype.readJointObject=function(a){var b=a.target.tag.skeleton.bones[a.joint];this.joints=this.joints||[];this.joints.push(this.objects["jnt/"+a.name]=a.tag=b)};
THREE.SEA3D.prototype.getSkeletonAnimation=function(a,b){if(a.tag)return a.tag;for(var c=[],d=a.frameRate/1E3,e=[1,1,1],f=new THREE.Matrix4,h=0;h<a.sequence.length;h++){for(var g=a.sequence[h],k=g.start,l=k+g.count,g={name:a.name+"/"+g.name,repeat:g.repeat,fps:a.frameRate,JIT:0,length:d*(g.count-1),hierarchy:[]},n=a.pose[0].length,q=0;q<n;q++){for(var u=b.joint[q],t={parent:u.parentIndex,keys:[]},x=t.keys,y=0,v=k;v<l;v++){var r=a.pose[v][q],p=THREE.SEA3D.BUFFER2.makeRotationFromQuaternion(new THREE.Quaternion(r.qx,
r.qy,r.qz,r.qw));p.setPosition(new THREE.Vector3(r.x,r.y,r.z));-1<u.parentIndex?(f.elements=b.joint[u.parentIndex].inverseBindMatrix,r=THREE.SEA3D.BUFFER3.getInverse(f),p.multiplyMatrices(r,p),this.flipMatrix(p),r.getInverse(f),this.flipMatrix(r),r.getInverse(r),p.multiplyMatrices(r,p)):this.flipMatrix(p);r=THREE.SEA3D.VECBUF0.setFromMatrixPosition(p);p=THREE.SEA3D.QUABUF0.setFromRotationMatrix(p);x.push({time:y,pos:[r.x,r.y,r.z],rot:[p.x,p.y,p.z,p.w],scl:e});y+=d}g.hierarchy[q]=t}c.push(g)}return a.tag=
c};THREE.SEA3D.prototype.getMorpher=function(a,b){for(var c=[],d=this.config.flip?-1:1,e=0;e<a.node.length;e++){for(var f=a.node[e],h=[],g=0,k=0;g<b.vertex.length;)h[k++]=new THREE.Vector3(b.vertex[g]+f.vertex[g++],b.vertex[g]+f.vertex[g++],(b.vertex[g]+f.vertex[g++])*d);c[f.name]=e;c[e]={name:f.name,vertices:h}}return c};THREE.SEA3D.Event={LOAD_PROGRESS:"sea3d_progress",DOWNLOAD_PROGRESS:"sea3d_download",COMPLETE:"sea3d_complete",OBJECT_COMPLETE:"sea3d_object",ERROR:"sea3d_error"};
...
this.readLine;this.file.typeRead[SEA3D.Material.prototype.type]=this.readMaterial;this.file.typeRead[SEA3D.PointLight.prototype.type]=this.readPointLight;this.file.typeRead[SEA3D.DirectionalLight.prototype.type]=this.readDirectionalLight;this.file.typeRead[SEA3D.HemisphereLight.prototype.type]=this.readHemisphereLight;this.file.typeRead[SEA3D.Camera.prototype.type]=this.readCamera;this.file.typeRead[SEA3D.Skeleton.prototype.type]=this.readSkeleton;this.file.typeRead[SEA3D.SkeletonLocal.prototype.type]=
this.readSkeletonLocal;this.file.typeRead[SEA3D.JointObject.prototype.type]=this.readJointObject;this.file.typeRead[SEA3D.CubeMap.prototype.type]=this.readCubeMap;this.file.typeRead[SEA3D.CubeRender.prototype.type]=this.readCubeRender;this.file.typeRead[SEA3D.Animation.prototype.type]=this.readAnimation;this.file.typeRead[SEA3D.SoundPoint.prototype.type]=this.readSoundPoint;this.file.typeRead[SEA3D.TextureURL.prototype.type]=this.readTextureURL;this.file.typeRead[SEA3D.JPEG.prototype.type]=this.readImage;
this.file.typeRead[SEA3D.JPEG_XR.prototype.type]=this.readImage;this.file.typeRead[SEA3D.PNG.prototype.type]=this.readImage;this.file.typeRead[SEA3D.GIF.prototype.type]=this.readImage;this.file.typeRead[SEA3D.MP3.prototype.type]=this.readSound;this.file.typeRead[SEA3D.GLSL.prototype.type]=this.readGLSL;this.file.typeRead[SEA3D.JavaScript.prototype.type]=this.readJavaScript;a&&this.file.read()};