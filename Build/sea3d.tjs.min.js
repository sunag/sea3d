...
4<=b)if(c=(b>>1)-1,g=(2|b&1)<<c,14>b)g+=a.reverseDecode2(this._posDecoders,g-b-1,this._rangeDecoder,c);else{if(g+=this._rangeDecoder.decodeDirectBits(c-4)<<4,g+=this._posAlignDecoder.reverseDecode(this._rangeDecoder),0>g){if(-1===g)break;return!1}}else g=b;if(g>=m||g>=this._dictionarySizeCheck)return!1;this._outWindow.copyBlock(g,l);m+=l;l=this._outWindow.getByte(0)}this._outWindow.flush();this._outWindow.releaseStream();this._rangeDecoder.releaseStream();return!0};a.Decoder.prototype.setDecoderProperties=
...
THREE.SEA3D.prototype.readMesh=function(a){var b,c,d=a.geometry.tag,e,g,h,f,k,m,l;b=0;for(c=a.modifiers?a.modifiers.length:0;b<c;b++){var n=a.modifiers[b];switch(n.type){case SEA3D.Skeleton.prototype.type:case SEA3D.SkeletonLocal.prototype.type:g=n;d.bones=g.tag;break;case SEA3D.Morph.prototype.type:h=n.tag||this.getModifier({sea:n,geometry:a.geometry}),d.morphAttributes=h.attribs,d.morphTargets=h.targets}}b=0;for(c=a.animations?a.animations.length:0;b<c;b++)switch(n=a.animations[b].tag,n.type){case SEA3D.SkeletonAnimation.prototype.type:f=
n;d.animations=f.tag||this.getModifier({sea:f,skeleton:g,relative:!0});break;case SEA3D.VertexAnimation.prototype.type:k=n;d.morphAttributes=k.tag.attribs;d.morphTargets=k.tag.targets;d.animations=k.tag.animations;break;case SEA3D.UVWAnimation.prototype.type:m=n.tag||this.getModifier({sea:n});break;case SEA3D.MorphAnimation.prototype.type:l=n.tag||this.getModifier({sea:n})}c=void 0!=h||void 0!=k;h=h&&void 0!=h.attribs.normal||k&&void 0!=k.tag.attribs.normal;if(a.material)if(1<a.material.length)for(e=
[],b=0;b<a.material.length;b++)e[b]=a.material[b].tag,e[b].skinning=void 0!=g,e[b].morphTargets=c,e[b].morphNormals=h,e[b].vertexColors=a.geometry.color?THREE.VertexColors:THREE.NoColors;else e=a.material[0].tag,e.skinning=void 0!=g,e.morphTargets=c,e.morphNormals=h,e.vertexColors=a.geometry.color?THREE.VertexColors:THREE.NoColors;g?(b=new THREE.SEA3D.SkinnedMesh(d,e,this.config.useVertexTexture),this.config.autoPlay&&f&&b.play(0)):k?(b=new THREE.SEA3D.VertexAnimationMesh(d,e),this.config.autoPlay&&
...
THREE.SEA3D.prototype.readAnimation=function(a){for(var b=[],c=1E3/a.frameRate/1E3,d=0;d<a.sequence.length;d++){for(var e=a.sequence[d],g=[],h=0;h<a.dataList.length;h++){var f=a.dataList[h],k,m,l,n=f.data;l=e.start*f.blockSize;k=l+e.count*f.blockSize;var q=e.intrpl?THREE.InterpolateLinear:!1,p=null;switch(f.kind){case SEA3D.Animation.POSITION:p=".position";break;case SEA3D.Animation.ROTATION:p=".quaternion";break;case SEA3D.Animation.SCALE:p=".scale";break;case SEA3D.Animation.COLOR:p=".color";break;
...
n.subarray(l,k);m=new Float32Array(l.length);for(k=f=0;k<m.length;k++)m[k]=f,f+=c;g.push(new THREE.NumberKeyframeTrack(p,m,l,q));break;case SEA3D.Stream.VECTOR3D:l=n.subarray(l,k);m=new Float32Array(l.length/f.blockSize);for(k=f=0;k<m.length;k++)m[k]=f,f+=c;g.push(new THREE.VectorKeyframeTrack(p,m,l,q));break;case SEA3D.Stream.VECTOR4D:l=n.subarray(l,k);m=new Float32Array(l.length/f.blockSize);for(k=f=0;k<m.length;k++)m[k]=f,f+=c;g.push(new THREE.QuaternionKeyframeTrack(p,m,l,q));break;case SEA3D.Stream.INT24:case SEA3D.Stream.UINT24:l=
new Float32Array(3*(k-l));m=new Float32Array(l.length/3);for(k=f=0;k<m.length;k++)l[3*k]=(n[k]>>16&255)/255,l[3*k+1]=(n[k]>>8&255)/255,l[3*k+2]=(n[k]&255)/255,m[k]=f,f+=c;g.push(new THREE.VectorKeyframeTrack(p,m,l,q))}}b.push(new THREE.SEA3D.AnimationClip(e.name,-1,g,e.repeat))}this.domain.clips=this.clips=this.clips||[];this.clips.push(this.objects[a.name+".anm"]=a.tag=b)};
THREE.SEA3D.prototype.readSkeletonAnimation=function(a,b){b=!b&&a.metadata&&a.metadata.skeleton?a.metadata.skeleton:b;if(!b||a.tag)return a.tag;for(var c=[],d=1E3/a.frameRate/1E3,e=0;e<a.sequence.length;e++){for(var g=a.sequence[e],h=g.start,f=h+g.count,k={name:g.name,fps:a.frameRate,length:d*g.count,hierarchy:[]},m=a.numJoints,l=a.raw,n=0;n<m;n++){for(var q={parent:b.joint[n].parentIndex,keys:[]},p=q.keys,u=0,t=h;t<f;t++){var r=t*m*7+7*n;p.push({time:u,pos:[l[r],l[r+1],l[r+2]],rot:[l[r+3],l[r+4],
l[r+5],l[r+6]],scl:[1,1,1]});u+=d}k.hierarchy[n]=q}c.push(THREE.SEA3D.AnimationClip.fromClip(THREE.AnimationClip.parseAnimation(k,b.tag),g.repeat))}this.domain.clips=this.clips=this.clips||[];this.clips.push(this.objects[a.name+".skla"]=a.tag=c)};
