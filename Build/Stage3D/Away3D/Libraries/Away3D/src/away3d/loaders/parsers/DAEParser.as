package away3d.loaders.parsers
{
	...
	public class DAEParser extends ParserBase
	{
			...
			for (i = 0; i < node.instance_controllers.length; i++) {
				...
				if (controller.skin && controller.skin.userData is Skeleton) {
					...
					if (!animationSet)
					{
						...
						animationSet.name = controller.skin.source;
					}
		...
		private function processSkinAnimation(skin:DAESkin, mesh:Mesh, skeleton:Skeleton):SkeletonClipNode
		{
			...
			clip.frameRate = Math.round( numFrames / duration );
		...
		private function parseSkeleton(instance_controller:DAEInstanceController):Skeleton
		{
			...
			skeleton.name = controller.skin.source;
...
class DAEElement
{
	...
	protected function readIntArray(element:XML, ints:Vector.<int>=null):Vector.<int>
	{
		ints ||= new Vector.<int>();
...
class DAEPrimitive extends DAEElement
{
	...
	public override function deserialize(element:XML):void
	{
		...
		// fix: Sunag Entertainment - Autodesk polygons mods+
		if (element.ns::p && element.ns::p.length())
		{
			i = 0;
			_p = new Vector.<int>();      
			while (i < element.ns::p.length())
			{
				readIntArray(element.ns::p[i++], _p);
			}
		}
		...
		{
			i = 0;
			_vcount = new Vector.<int>();  
			while (i < element.ns::vcount.length())
			{
				readIntArray(element.ns::vcount[i++], _vcount);
			}
		}
	}
