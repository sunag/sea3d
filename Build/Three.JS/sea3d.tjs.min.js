...
SEA3D.Math.lerpAng1x=function(a,b,c){a[0]=SEA3D.Math.lerpAngle(a[0],b[0],c)};SEA3D.Math.lerpColor1x=function(a,b,c){a[0]=SEA3D.Math.lerpColor(a[0],b[0],c)};SEA3D.Math.lerpQuat4x=function(a,b,c){var d=a[0],e=a[1],g=a[2],h=a[3],f=b[0],k=b[1],m=b[2];b=b[3];0>d*f+e*k+g*m+h*b&&(f=-f,k=-k,m=-m,b=-b);d+=c*(f-d);e+=c*(k-e);g+=c*(m-g);c=h+c*(b-h);h=1/Math.sqrt(c*c+d*d+e*e+g*g);a[0]=d*h;a[1]=e*h;a[2]=g*h;a[3]=c*h};SEA3D.Timer=function(){this.time=this.start=Date.now()};
...
THREE.SEA3D.prototype.readAnimation=function(a){for(var b=[],c=1E3/a.frameRate/1E3,d=0;d<a.sequence.length;d++){for(var e=a.sequence[d],g=[],h=0;h<a.dataList.length;h++){var f=a.dataList[h],k,m,l,p=f.data;l=e.start*f.blockSize;k=l+e.count*f.blockSize;var q=e.intrpl?THREE.InterpolateLinear:!1,n=null;switch(f.kind){case SEA3D.Animation.POSITION:n=".position";break;case SEA3D.Animation.ROTATION:n=".quaternion";break;case SEA3D.Animation.SCALE:n=".scale";break;case SEA3D.Animation.COLOR:n=".color";break;
case SEA3D.Animation.MULTIPLIER:n=".intensity";break;case SEA3D.Animation.FOV:n=".fov"}if(n)switch(f.type){case SEA3D.Stream.BYTE:case SEA3D.Stream.UBYTE:case SEA3D.Stream.INT:case SEA3D.Stream.UINT:case SEA3D.Stream.FLOAT:case SEA3D.Stream.DOUBLE:case SEA3D.Stream.DECIMAL:l=p.subarray(l,k);m=new Float32Array(l.length);for(k=f=0;k<m.length;k++)m[k]=f,f+=c;g.push(new THREE.VectorKeyframeTrack(n,m,l,q));break;case SEA3D.Stream.VECTOR3D:l=p.subarray(l,k);m=new Float32Array(l.length/f.blockSize);for(k=
f=0;k<m.length;k++)m[k]=f,f+=c;g.push(new THREE.VectorKeyframeTrack(n,m,l,q));break;case SEA3D.Stream.VECTOR4D:l=p.subarray(l,k);m=new Float32Array(l.length/f.blockSize);for(k=f=0;k<m.length;k++)m[k]=f,f+=c;g.push(new THREE.QuaternionKeyframeTrack(n,m,l,q));break;case SEA3D.Stream.INT24:case SEA3D.Stream.UINT24:l=new Float32Array(3*(k-l));m=new Float32Array(l.length/3);for(k=f=0;k<m.length;k++)l[3*k]=(p[k]>>16&255)/255,l[3*k+1]=(p[k]>>8&255)/255,l[3*k+2]=(p[k]&255)/255,m[k]=f,f+=c;g.push(new THREE.VectorKeyframeTrack(n,
m,l,q))}}b.push(new THREE.SEA3D.AnimationClip(e.name,-1,g,e.repeat))}this.domain.clips=this.clips=this.clips||[];this.clips.push(this.objects[a.name+".anm"]=a.tag=b)};
THREE.SEA3D.prototype.readSkeletonAnimation=function(a,b){for(var c=[],d=1E3/a.frameRate/1E3,e=0;e<a.sequence.length;e++){for(var g=a.sequence[e],h=g.start,f=h+g.count,k={name:g.name,fps:a.frameRate,length:d*g.count,hierarchy:[]},m=a.numJoints,l=a.raw,p=0;p<m;p++){for(var q={parent:b.joint[p].parentIndex,keys:[]},n=q.keys,u=0,t=h;t<f;t++){var r=t*m*7+7*p;n.push({time:u,pos:[l[r],l[r+1],l[r+2]],rot:[l[r+3],l[r+4],l[r+5],l[r+6]],scl:[1,1,1]});u+=d}k.hierarchy[p]=q}c.push(THREE.SEA3D.AnimationClip.fromClip(THREE.AnimationClip.parseAnimation(k,
