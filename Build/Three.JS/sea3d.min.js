...
this.numColor=((c&64)>>6|(c&128)>>6)+1,this.color=[],a=0,c&=15;a<c;a++)this.color.push(b.readFloatArray(this.numVertex*this.numColor));this.vertex=b.readFloatArray(this.length);var d=b.readUByte();this.groups=[];if(this.attrib&2){for(c=a=0;a<d;a++)this.groups.push({start:3*b.readVInt(),count:c+=3*b.readVInt()});this.indexes=this.isBig?b.readUIntArray(c):b.readUShortArray(c)}else{var e=this.isBig?4:2,f=new SEA3D.UByteArray;for(a=0;a<d;a++)c=3*b.readVInt(),this.groups.push({start:0,count:c}),c+=0,f.add(b.readUByteArray(c*
