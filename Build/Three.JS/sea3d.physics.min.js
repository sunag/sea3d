...
b,c){b=void 0==b?-1:b;c=void 0==c?!1:c;var d=new Ammo.btTriangleMesh,k=new Ammo.btVector3(0,0,0),g=new Ammo.btVector3(0,0,0),e=new Ammo.btVector3(0,0,0),f=a.getAttribute("position").array,h=a.getIndex().array,l=0<=b?a.groups[b]:void 0;a=l?l.count:h.length;b=1/this.worldScale;for(l=l?l.start:0;l<a;l+=3){var m=3*h[l],n=3*h[l+1],p=3*h[l+2];k.setValue(f[m]*b,f[m+1]*b,f[m+2]*b);g.setValue(f[n]*b,f[n+1]*b,f[n+2]*b);e.setValue(f[p]*b,f[p+1]*b,f[p+2]*b);d.addTriangle(k,g,e,c)}return d},createConvexHull:function(a,
b){b=void 0==b?-1:b;var c=new Ammo.btConvexHullShape;new Ammo.btVector3(0,0,0);for(var d=a.getAttribute("position").array,k=a.getIndex().array,g=0<=b?a.groups[b]:void 0,e=g?g.count:k.length,f=1/this.worldScale,g=g?g.start:0;g<e;g+=3){var h=3*k[g],h=new Ammo.btVector3(d[h]*f,d[h+1]*f,d[h+2]*f);c.addPoint(h)}return c},getTargetByRigidBody:function(a){return this.rigidBodiesTarget[this.rigidBodies.indexOf(a)]},getRigidBodyByTarget:function(a){return this.rigidBodies[this.rigidBodiesTarget.indexOf(a)]},
...
g){g=g||new THREE.Matrix4;var e=a.getOrigin(),f=a.getRotation();b.set(e.x(),e.y(),e.z());c.set(f.x(),f.y(),f.z(),f.w());g.compose(b,c,d);return g}}(),updateTargetTransform:function(){var a=new THREE.Matrix4,b=new THREE.Vector3,c=new THREE.Quaternion,d=new THREE.Vector3(1,1,1);return function(k,g,e){var f=g.getOrigin();g=g.getRotation();e?(b.set(f.x(),f.y(),f.z()),c.set(g.x(),g.y(),g.z(),g.w()),a.compose(b,c,d),a.multiplyMatrices(a,e),k.position.setFromMatrixPosition(a),k.quaternion.setFromRotationMatrix(a)):
(k.position.set(f.x(),f.y(),f.z()),k.quaternion.set(g.x(),g.y(),g.z(),g.w()));return this}}(),update:function(a,b,c){this.world.stepSimulation(a,b||0,c||.06);for(a=0;a<this.vehicles.length;a++){c=this.vehicles[a];var d=c.getNumWheels(),k=this.vehiclesWheels[a];for(b=0;b<d;b++){c.updateWheelTransform(b,!0);var g=c.getWheelTransformWS(b),e=k[b];e&&this.updateTargetTransform(e,g,f.physics?e.physics.offset:null)}}for(a=0;a<this.rigidBodies.length;a++){b=this.rigidBodies[a];var f=this.rigidBodiesTarget[a];
f&&b.isActive()&&this.updateTargetTransform(f,b.getWorldTransform(),f.physics?f.physics.offset:null)}return this}};THREE.SEA3D.prototype.toAmmoVec3=function(a){return new Ammo.btVector3(a.x,a.y,a.z)};THREE.SEA3D.prototype.readSphere=function(a){var b=new Ammo.btSphereShape(a.radius);this.domain.shapes=this.shapes=this.shapes||[];this.shapes.push(this.objects["shpe/"+a.name]=a.tag=b)};THREE.SEA3D.prototype.readBox=function(a){var b=new Ammo.btBoxShape(new Ammo.btVector3(.5*a.width,.5*a.height,.5*a.depth));this.domain.shapes=this.shapes=this.shapes||[];this.shapes.push(this.objects["shpe/"+a.name]=a.tag=b)};
...
b.set_m_linearDamping(a.linearDamping);b.set_m_angularDamping(a.angularDamping);b=new Ammo.btRigidBody(b);d&&(d.physics.rigidBody=b,a.offset&&(c=new THREE.Matrix4,c.elements.set(a.offset),d.physics.offset=c));this.domain.rigidBodies=this.rigidBodies=this.rigidBodies||[];this.rigidBodies.push(this.objects["rb/"+a.name]=a.tag=b);return b};THREE.SEA3D.prototype.readRigidBody=function(a){var b=this.readRigidBodyBase(a);SEA3D.AMMO.addRigidBody(b,a.target?a.target.tag:void 0,this.config.enabledPhysics)};
...
var c=new Ammo.btRaycastVehicle(d,b,c),k=[];c.setCoordinateSystem(0,1,2);for(var g=0;g<a.wheel.length;g++){var e=a.wheel[g],f=c.addWheel(this.toAmmoVec3(e.pos),this.toAmmoVec3(e.dir),this.toAmmoVec3(e.axle),e.suspensionRestLength,e.radius,d,e.isFront),h=k[g]=e.target?e.target.tag:void 0;if(h){h.physics={enabled:!0,rigidBody:f};if(e.offset){var l=new THREE.Matrix4;l.elements.set(e.offset);h.physics.offset=l}h.parent&&h.parent.remove(h);this.container&&this.container.add(h)}f.set_m_suspensionStiffness(a.suspensionStiffness);
f.set_m_wheelsDampingRelaxation(a.dampingRelaxation);f.set_m_wheelsDampingCompression(a.dampingCompression);f.set_m_frictionSlip(a.frictionSlip)}SEA3D.AMMO.addVehicle(c,k);SEA3D.AMMO.addRigidBody(b,a.target?a.target.tag:void 0,this.config.enabledPhysics);this.domain.vehicles=this.vehicles=this.vehicles||[];this.vehicles.push(this.objects["vhc/"+a.name]=a.tag=c)};
...
THREE.SEA3D.Domain.prototype.applyTransform=function(a){for(var b=THREE.SEA3D.MTXBUF,c=THREE.SEA3D.VECBUF,d=this.rigidBodies?this.rigidBodies.length:0,k=this.container?this.container.children:[],g=[];d--;){var e=this.rigidBodies[d],f=SEA3D.AMMO.getTargetByRigidBody(e),h=e.getWorldTransform(),h=SEA3D.AMMO.getMatrixFromTransform(h);h.multiplyMatrices(h,a);h=SEA3D.AMMO.getTransformFromMatrix(h);e.setWorldTransform(h);f&&g.push(f)}for(d=0;d<k.length;d++)e=k[d],-1<g.indexOf(e)||(e.updateMatrix(),b.copy(e.matrix),
