package away3d.containers
{
	...
	public class ObjectContainer3D extends Object3D implements IAsset
	{
		...
		arcane var _oldScene:Scene3D;
		
		protected var _scene:Scene3D;
		protected var _parent:ObjectContainer3D;
		protected var _animateTransform : Matrix3D;
		...
		protected function updateSceneTransform():void
		{
			...
			if (_animateTransform)				
				_sceneTransform.prepend(_animateTransform);
		...
		public function set animateTransform(val:Matrix3D) : void
		{
			_animateTransform = val;
			notifySceneTransformChange();
		}
		
		public function get animateTransform() : Matrix3D
		{
		...
		public function getChildByName(name : String, useSubChildren:Boolean = false) : ObjectContainer3D
		{
			...
			if (useSubChildren)
			{
				for each(c in _children)
				{
					c = c.getChildByName(name, useSubChildren);
					if (c) return c;
				}
			}
		...
		public function get childrens():Vector.<ObjectContainer3D>
		{
			return _children;
		}
