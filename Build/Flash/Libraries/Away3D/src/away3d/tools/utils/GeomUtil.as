package away3d.tools.utils
{
	...
	public class GeomUtil
	{
		 ...
		 */
		public static function fromVectors(verts:Vector.<Number>, indices:Vector.<uint>, uvs:Vector.<Number>=null, normals:Vector.<Number>=null, tangents:Vector.<Number>=null, weights:Vector.<Number>=null, jointIndices:Vector.<Number>=null, uvs2:Vector.<Number>=null, triangleOffset:int = 0):Vector.<ISubGeometry>
		{
			const LIMIT_VERTS:uint = 3*0xfffe;
			const LIMIT_INDICES:uint = 15*0xfffe;
			...
			if ((indices.length >= LIMIT_INDICES) || (verts.length >= LIMIT_VERTS)) {
				...
				var splitUvs2:Vector.<Number> = (uvs2 != null)? new Vector.<Number>() : null;
				...
				for (i = 0; i < len; i += 3) {
					...
					if (( (outIndex + 2) >= LIMIT_INDICES) || (splitIndex >= LIMIT_VERTS)) {
						subs.push(constructSubGeometry(splitVerts, splitIndices, splitUvs, splitNormals, splitTangents, splitWeights, splitJointIndices, splitUvs2, triangleOffset));
				...
				subs.push(constructSubGeometry(verts, indices, uvs, normals, tangents, weights, jointIndices, uvs2, triangleOffset));
		 ...
		 */
		public static function constructSubGeometry(verts:Vector.<Number>, indices:Vector.<uint>, uvs:Vector.<Number>, normals:Vector.<Number>, tangents:Vector.<Number>, weights:Vector.<Number>, jointIndices:Vector.<Number>, uvs2:Vector.<Number>, triangleOffset:int):CompactSubGeometry
		{
			...
			sub.fromVectors(verts, uvs, normals, tangents, uvs2);
			return sub;
		}
