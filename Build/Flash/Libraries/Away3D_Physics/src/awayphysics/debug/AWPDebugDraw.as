package awayphysics.debug 
{
	...
	public class AWPDebugDraw 
	{
		...
		public static const DBG_TriangleShapes : int = 32;
		...
		private function drawTriangles(geometry:Geometry, scale:Vector3D, transform:AWPTransform, color:uint):void {
			if (m_debugMode & DBG_TriangleShapes != 0)				
				return;
			...
			if(geometry.subGeometries[0] is CompactSubGeometry){
				vertexDataNum = 13;
			}else if(geometry.subGeometries[0] is SubGeometry){
				vertexDataNum = 3;
			}
			...
			for (var i:int = 0; i < indexDataLen; i += 3 ) {
				v0.setTo(vertexData[vertexDataNum*indexData[m]] * scale.x, vertexData[vertexDataNum*indexData[m]+1] * scale.y, vertexData[vertexDataNum*indexData[m]+2] * scale.z);
				m++;
				v1.setTo(vertexData[vertexDataNum*indexData[m]] * scale.x, vertexData[vertexDataNum*indexData[m]+1] * scale.y, vertexData[vertexDataNum*indexData[m]+2] * scale.z);
				m++;
				v2.setTo(vertexData[vertexDataNum*indexData[m]] * scale.x, vertexData[vertexDataNum*indexData[m]+1] * scale.y, vertexData[vertexDataNum*indexData[m]+2] * scale.z);
				m++;
