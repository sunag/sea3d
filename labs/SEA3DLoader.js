...
THREE.SEA3D.prototype.readGeometry = function(sea) {
	...
	vertex = geo.vertices = this.flipX ? this.vectorToVector3Inv(sea.vertex) : this.vectorToVector3(sea.vertex);	
	if (sea.normal) normal = this.flipX ? this.vectorToVector3Inv(sea.normal) : this.vectorToVector3(sea.normal);
...
THREE.SEA3D.prototype.readSkeleton = function(sea) {		
	...
	for (var i = 0; i < sea.joint.length; i++)
	{
		...
		if (bone.parentIndex > -1)
		{
			...
			mtx_inv = mtx_inv.clone();
			
			mtx_inv.getInverse( mtx_inv.clone() );									
			
			//--
			
			mtx_inv = flipMtx(mtx_inv);
			
			//--
			
			mtx_inv.getInverse( mtx_inv.clone() );									
			
			mtx.multiplyMatrices( mtx_inv.clone(), mtx.clone() );				
		}
...
function flipMtx(mtx)
{
	var newmtx = new THREE.Matrix4();
	
	var nmtx = mtx.clone();
	nmtx.setPosition( new THREE.Vector3() );	
	nmtx.multiplyMatrices( new THREE.Matrix4().makeRotationAxis( new THREE.Vector3(1,0,0), THREE.Math.degToRad( 180 ) ), nmtx );	
	var quat = new THREE.Quaternion().setFromRotationMatrix( nmtx );			
	
	newmtx.makeRotationFromQuaternion(quat);	
	
	var pos = new THREE.Vector3().setFromMatrixPosition( mtx );			
	pos.x = -pos.x;
	newmtx.setPosition(pos);										
	...
	for (var i = 0; i < sea.sequence.length; i++) {
		...
		for (var j = 0; j < len; j++) {			
			...
			for (var t = start; t < end; t++) {	
				...
				var mtx_global = new THREE.Matrix4().makeRotationFromQuaternion(new THREE.Quaternion(joint.qx, joint.qy, joint.qz, joint.qw));
				...
				if (bone.parentIndex > -1)
				{
					...
					var mtx_rect = new THREE.Matrix4();
					...
					mtx_global.multiplyMatrices( mtx_inv, mtx_global );
				}
				else
				{
					mtx_global = flipMtx(mtx_global);
				...
				var posQ = new THREE.Vector3().setFromMatrixPosition(mtx_global);
				var newQ = new THREE.Quaternion().setFromRotationMatrix(mtx_global);
