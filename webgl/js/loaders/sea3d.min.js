/** SEA3D - (C) Sunag Entertainment 2014 - http://www.sunag.com.br/ */
...
SEA3D.JointObject.prototype.type="jnt";SEA3D.Light={read:function(scope){SEA3D.Object3D.read(scope);var data=scope.data;scope.attenStart=Number.MAX_VALUE;scope.attenEnd=Number.MAX_VALUE;if(scope.attrib&64){var shadowHeader=data.readUByte();scope.shadow={}
scope.shadow.opacity=shadowHeader&1?data.readFloat():1;scope.shadow.color=shadowHeader&2?data.readUInt24():0x000000;}
if(scope.attrib&512){scope.attenStart=data.readFloat();scope.attenEnd=data.readFloat();}
...
SEA3D.File.prototype.readStage=function(scope){while(scope.stage&&scope.stage());if(scope.stage){window.setInterval(scope.readStage,10,scope);scope.dispatchProgress();}}
SEA3D.File.prototype.read=function(){this.timer=new SEA3D.Timer();this.stage=this.readHead;this.readStage(this);}
...
THREE.SEA3D.prototype.tangent=true;THREE.SEA3D.prototype.bounding=true;THREE.SEA3D.prototype.autoPlay=true;THREE.SEA3D.prototype.matrixAutoUpdate=true;THREE.SEA3D.prototype.parser=THREE.SEA3D.AUTO;THREE.SEA3D.prototype.getMesh=function(name){return this.objects["m3d/"+name];}
...
{material=sea.material.tag.sprite=new THREE.SpriteMaterial();material.map=sea.material.tag.map;material.map.flipY=true;material.color=sea.material.tag.emissive;material.opacity=sea.material.tag.opacity;material.blending=sea.material.tag.blending;}
...
techniques[SEA3D.Material.DEFAULT]=function(tech,mat){mat.ambient.setHex(tech.ambientColor);mat.emissive.setHex(tech.ambientColor);mat.color.setHex(tech.diffuseColor);mat.specular.setHex(this.scaleColor(tech.specularColor,tech.specular));mat.shininess=tech.gloss;}
...
THREE.SEA3D.prototype.readPointLight=function(sea){var light=new THREE.PointLight(sea.color,sea.multiplier);light.name=sea.name;light.position.set(sea.position.x,sea.position.y,this.invertZ&&!sea.parent?-sea.position.z:sea.position.z);if(sea.shadow)
this.setShadowMap(light,sea.shadow.opacity);this.lights=this.lights||[];this.lights.push(this.objects["lht/"+sea.name]=sea.tag=light);this.addSceneObject(sea);this.applyDefaultAnimation(sea,THREE.SEA3D.LightAnimator);}
