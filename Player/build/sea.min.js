...
case SEA3D.Animation.MULTIPLIER:r=".intensity";break;case SEA3D.Animation.FOV:r=".fov"}if(r)switch(g.type){case SEA3D.Stream.BYTE:case SEA3D.Stream.UBYTE:case SEA3D.Stream.INT:case SEA3D.Stream.UINT:case SEA3D.Stream.FLOAT:case SEA3D.Stream.DOUBLE:case SEA3D.Stream.DECIMAL:l=q.subarray(l,k);m=new Float32Array(l.length);for(k=g=0;k<m.length;k++)m[k]=g,g+=c;f.push(new THREE.VectorKeyframeTrack(r,m,l,s));break;case SEA3D.Stream.VECTOR3D:l=q.subarray(l,k);m=new Float32Array(l.length/g.blockSize);for(k=
g=0;k<m.length;k++)m[k]=g,g+=c;f.push(new THREE.VectorKeyframeTrack(r,m,l,s));break;case SEA3D.Stream.VECTOR4D:l=q.subarray(l,k);m=new Float32Array(l.length/g.blockSize);for(k=g=0;k<m.length;k++)m[k]=g,g+=c;f.push(new THREE.QuaternionKeyframeTrack(r,m,l,s));break;case SEA3D.Stream.INT24:case SEA3D.Stream.UINT24:l=new Float32Array(3*(k-l));m=new Float32Array(l.length/3);for(k=g=0;k<m.length;k++)l[3*k]=(q[k]>>16&255)/255,l[3*k+1]=(q[k]>>8&255)/255,l[3*k+2]=(q[k]&255)/255,m[k]=g,g+=c;f.push(new THREE.VectorKeyframeTrack(r,
m,l,s))}}f=new THREE.AnimationClip(e.name,-1,f);f.loop=e.repeat;f.timeScale=1;b.push(f)}this.domain.animationClips=this.animationClips=this.animationClips||[];this.animationClips.push(this.objects[a.name+".#anm"]=a.tag=b)};
...
THREE.SEA3D.prototype.getSkeletonAnimation=function(a,b){if(a.tag)return a.tag;for(var c=[],d=1E3/a.frameRate/1E3,e=0;e<a.sequence.length;e++){for(var f=a.sequence[e],h=f.start,g=h+f.count,k={name:f.name,fps:a.frameRate,length:d*f.count,hierarchy:[]},m=a.numJoints,l=a.raw,q=0;q<m;q++){for(var s={parent:b.joint[q].parentIndex,keys:[]},r=s.keys,p=0,n=h;n<g;n++){var t=n*m*7+7*q;r.push({time:p,pos:[l[t],l[t+1],l[t+2]],rot:[l[t+3],l[t+4],l[t+5],l[t+6]],scl:[1,1,1]});p+=d}k.hierarchy[q]=s}h=THREE.AnimationClip.parseAnimation(k,
...
THREE.SEA3D.prototype.isLegacy=function(a){var b=a.sea3d;return"S3D"!=b.sign||a._legacy?!1:(a._legacy=!0==b.typeUnique[a.type],b.config.legacy)};THREE.SEA3D.prototype.flipXVec3=function(a){if(a){for(var b=0;b<a.length;)a[b]=-a[b],b+=3;return a}};
...
THREE.SEA3D.prototype.flipYZIndex=function(a){for(var b=1;b<a.length;){var c=a[b+1];a[b+1]=a[b];a[b]=c;b+=3}return a};THREE.SEA3D.prototype.flipMatrixBone=function(){var a=new THREE.Vector3;return function(b){var c=THREE.SEA3D.VECBUF.setFromMatrixPosition(b);c.x=-c.x;b.setPosition(a);b.multiplyMatrices(THREE.SEA3D.MTXBUF.makeRotationX(THREE.Math.degToRad(180)),b);b.setPosition(c);return b}}();
THREE.SEA3D.prototype.flipMatrixScale=function(){var a=new THREE.Vector3,b=new THREE.Quaternion,c=new THREE.Vector3;return function(d,e,f,h){f&&d.multiplyMatrices(f,d);d.decompose(a,b,c);c.x=-c.x;d.compose(a,b,c);f&&(f=f.clone(),this.flipMatrixScale(f,h),d.multiplyMatrices(f.getInverse(f),d));return d}}();
THREE.SEA3D.prototype.updateAnimationSet=function(){var a=new THREE.Matrix4,b=new THREE.Matrix4,c=new THREE.Vector3,d=new THREE.Quaternion,e=new THREE.Vector3,f=new THREE.Vector3,h=new THREE.Quaternion,g=new THREE.Vector3;return function(k){var m=k.animation.animationSet,l=m.animations;if(!m.flip||l.length){for(var q=l[0].dataList,l=[],s=0;s<q.length;s++){var r=q[s],p=q[s].data,n=r.kind,r=p.length/r.blockLength;switch(n){case SEA3D.Animation.POSITION:case SEA3D.Animation.ROTATION:case SEA3D.Animation.SCALE:l.push({kind:n,
numFrames:r,raw:p})}}if(0<l.length)for(r=l[0].numFrames,p=void 0,k.animation.relative?(a.identity(),this.flipMatrixScale(b.copy(k.matrixWorld))):(k.parent&&(p=this.flipMatrixScale(b.copy(k.parent.matrixWorld))),this.flipMatrixScale(a.copy(k.matrix),!1,p)),a.decompose(c,d,e),k=0;k<r;k++){for(q=0;q<l.length;q++)switch(p=l[q].raw,n=l[q].kind,n){case SEA3D.Animation.POSITION:n=3*k;c.set(p[n],p[n+1],p[n+2]);break;case SEA3D.Animation.ROTATION:n=4*k;d.set(p[n],p[n+1],p[n+2],p[n+3]);break;case SEA3D.Animation.SCALE:n=
4*k,e.set(p[n],p[n+1],p[n+2])}a.compose(c,d,e);this.flipMatrixScale(a,!1,b);a.decompose(f,h,g);for(q=0;q<l.length;q++)switch(p=l[q].raw,n=l[q].kind,n){case SEA3D.Animation.POSITION:n=3*k;p[n]=f.x;p[n+1]=f.y;p[n+2]=f.z;break;case SEA3D.Animation.ROTATION:n=4*k;p[n]=h.x;p[n+1]=h.y;p[n+2]=h.z;p[n+3]=h.w;break;case SEA3D.Animation.SCALE:n=3*k,p[n]=g.x,p[n+1]=g.y,p[n+2]=g.z}}m.flip=!0}}}();
...
e.setFromRotationMatrix(b);h[k]={name:m.name,pos:[d.x,d.y,d.z],rotq:[e.x,e.y,e.z,e.w],parent:m.parentIndex}}return f.tag=h}}();THREE.SEA3D.prototype.getSkeletonAnimation=function(a,b){return this.isLegacy(a)?this.getSkeletonAnimationLegacy(a,b):this._getSkeletonAnimation(a,b)};
THREE.SEA3D.prototype.getSkeletonAnimationLegacy=function(){var a=new THREE.Matrix4,b=new THREE.Matrix4,c=new THREE.Matrix4,d=new THREE.Matrix4;return function(e,f){if(e.tag)return e.tag;for(var h=[],g=1E3/e.frameRate/1E3,k=[1,1,1],m=0;m<e.sequence.length;m++){for(var l=e.sequence[m],q=l.start,s=q+l.count,r={name:l.name,repeat:l.repeat,fps:e.frameRate,JIT:0,length:g*l.count,hierarchy:[]},p=e.numJoints,n=e.raw,t=0;t<p;t++){for(var w=f.joint[t],z={parent:w.parentIndex,keys:[]},A=z.keys,x=0,y=q;y<s;y++){var u=
y*p*7+7*t;b.makeRotationFromQuaternion(THREE.SEA3D.QUABUF.set(n[u+3],n[u+4],n[u+5],n[u+6]));b.setPosition(THREE.SEA3D.VECBUF.set(n[u],n[u+1],n[u+2]));-1<w.parentIndex?(a.elements=f.joint[w.parentIndex].inverseBindMatrix,d.getInverse(a),c.multiplyMatrices(d,b),this.flipMatrixBone(c),this.flipMatrixBone(d),d.getInverse(d),b.multiplyMatrices(d,c)):this.flipMatrixBone(b);var u=THREE.SEA3D.VECBUF.setFromMatrixPosition(b),v=THREE.SEA3D.QUABUF.setFromRotationMatrix(b);A.push({time:x,pos:[u.x,u.y,u.z],rot:[v.x,
v.y,v.z,v.w],scl:k});x+=g}r.hierarchy[t]=z}q=THREE.AnimationClip.parseAnimation(r,f.tag);q.loop=l.repeat;q.timeScale=1;h.push(q)}return e.tag=h}}();THREE.SEA3D.prototype.readVertexAnimation=function(a){if(this.isLegacy(a))for(var b=0,c=a.frame.length;b<c;b++){var d=a.frame[b];this.flipXVec3(d.vertex);this.flipXVec3(d.normal)}this._readVertexAnimation(a)};
THREE.SEA3D.prototype.readGeometryBuffer=function(a){this.isLegacy(a)&&(this.flipXVec3(a.vertex),this.flipXVec3(a.normal),this.flipYZIndex(a.indexes),4<a.jointPerVertex?this.compressJoints(a):4>a.jointPerVertex&&this.expandJoints(a));this._readGeometryBuffer(a)};THREE.SEA3D.prototype.readLines=function(a){this.isLegacy(a)&&this.flipXVec3(a.vertex);this._readLines(a)};THREE.SEA3D.prototype.onHead=function(a){if("S3D"!=a.sign&&"TJS"!=a.sign)throw Error("Sign '"+a.sign+"' unknown.");};
