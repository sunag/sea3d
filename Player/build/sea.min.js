...
case SEA3D.Animation.MULTIPLIER:s=".intensity";break;case SEA3D.Animation.FOV:s=".fov"}if(s)switch(g.type){case SEA3D.Stream.BYTE:case SEA3D.Stream.UBYTE:case SEA3D.Stream.INT:case SEA3D.Stream.UINT:case SEA3D.Stream.FLOAT:case SEA3D.Stream.DOUBLE:case SEA3D.Stream.DECIMAL:l=q.subarray(l,k);m=new Float32Array(l.length);for(k=g=0;k<m.length;k++)m[k]=g,g+=c;f.push(new THREE.VectorKeyframeTrack(s,m,l,r));break;case SEA3D.Stream.VECTOR3D:l=q.subarray(l,k);m=new Float32Array(l.length/g.blockSize);for(k=
g=0;k<m.length;k++)m[k]=g,g+=c;f.push(new THREE.VectorKeyframeTrack(s,m,l,r));break;case SEA3D.Stream.VECTOR4D:l=q.subarray(l,k);m=new Float32Array(l.length/g.blockSize);for(k=g=0;k<m.length;k++)m[k]=g,g+=c;f.push(new THREE.QuaternionKeyframeTrack(s,m,l,r));break;case SEA3D.Stream.INT24:case SEA3D.Stream.UINT24:l=new Float32Array(3*(k-l));m=new Float32Array(l.length/3);for(k=g=0;k<m.length;k++)l[3*k]=(q[k]>>16&255)/255,l[3*k+1]=(q[k]>>8&255)/255,l[3*k+2]=(q[k]&255)/255,m[k]=g,g+=c;f.push(new THREE.VectorKeyframeTrack(s,
